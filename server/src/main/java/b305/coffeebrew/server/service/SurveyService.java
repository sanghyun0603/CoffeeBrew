...
public class SurveyService {
    ...
    public String registSurvey(SurveyReqDTO surveyReqDTO, Long memberId) {
        ...
        String[] params7_note = {"", "사과, 배", "체리, 복숭아", "크랜베리, 블루베리, 라즈베리", "레몬, 오렌지, 귤", "망고, 바나나, 파인애플"};
        // 우디, 시트러스, 허브, 꽃, 허니
        ...
        for (int i = 0; i < status.length; i++) {
            ...
            status[i] = (int)Math.round((double)status[i] / (12 * 6) * 10);
        }
        ...
        for (int i = 0; i < status.length; i++) {
            if (status[i] > status[idx1st]) {
                idx2nd = idx1st;
                idx1st = i;
            } else if (status[i] > status[idx2nd]) {
                idx2nd = i;
            }
        }
        idx1st++;
        ...
        String resultType = "Type"+String.valueOf(idx1st)+String.valueOf(idx2nd);

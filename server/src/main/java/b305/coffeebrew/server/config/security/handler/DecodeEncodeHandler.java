...
public class DecodeEncodeHandler {
	...
	public String roleValid(String memberId) {
		...
		Optional<Member> optionalMember = memberRepository.findByMemberEmailAAndExpiredIsFalse(memberId);
	...
	public boolean memberIdValid(String memberId) {
		...
		try {
			Optional<Member> member = memberRepository.findByMemberEmailAAndExpiredIsFalse(memberId);
			if (member.isPresent()) {
